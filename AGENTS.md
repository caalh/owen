...
MCNP, OpenMC, Serpent, and SCONE input files. Current release: **v1.0.0** (stable, built
locally as VSIX; GitHub Release v1.0.0). **v0.3.1 remains the published Marketplace /
Open VSX version** until the maintainer publishes 1.0.0.
It is a **BelvoirDynamics** product; its siblings are the **GROVES** desktop app (v1.3.4) and
the **NICHOLS** Sublime Text / Notepad++ packages (v1.0.0, `caalh/nichols`).

## Key modules (v1.0)

| Area | Path | Notes |
|------|------|-------|
| MC Language Server | `src/server/` + `src/lsp/client.ts` | bundled to `out/server.js`; other editors can run it over stdio |
