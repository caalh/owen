...
export type { DeckLoadOptions, ViewFit } from './deckLoader';
export { parseDeckArgs } from './deckLoader';
...
export interface RenderRequest {
    ...
    deckArgs: string[];
    /** Which bounding box an automatic view frames. */
    fit: ViewFit;
}
...
export function buildRenderRequest(
    ...
    return { deckPath, outDir, plots, ...normalizeDeckOptions(opts) };
}
...
def render(req, out_dir):
    ...
    ctx = owen_load_deck(req, out_dir, RESULT['warnings'], 'render-only pass')
    geometry = ctx['geometry']
    real_plot_geometry = ctx['realPlotGeometry']
    RESULT['modelSource'] = ctx['source']
